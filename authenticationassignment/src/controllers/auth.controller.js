...
body("name").exists().isLength({min:3, max:10}).withMessage("name is required and has to be at least 3 characters"),
body("email").exists().isEmail().withMessage("email is required"),
body("password").exists().isLength({min:8}).withMessage("Password length must be at least 8 characters")
,async(req,res)=>{
    // try{
